...
function Wit(opts) {
  ...
  const continueRunActionsZalo = (sessionId, currentRequest, message, prevContext, i) => {

  	//console.log("message 22222 :::: "+message)
     ...
     return (json) => {
      ...
      if (json.type === 'msg') {
        ...
        return runActionsZalo(actions, 'send', request, response).then(ctx => {
          if (ctx) {
            throw new Error('Cannot update context after \'send\' action');
          }
          if (currentRequest !== this._sessions[sessionId]) {
          ...
          return this.converse(sessionId, null, prevContext).then(
            continueRunActionsZalo(sessionId, currentRequest, message, prevContext, i - 1)
          );
        });
      } else if (json.type === 'action') {
        return runActionsZalo(actions, json.action, request).then(ctx => {
          const nextContext = ctx || {};
          if (currentRequest !== this._sessions[sessionId]) {
            return nextContext;
          }
          return this.converse(sessionId, null, nextContext).then(
            continueRunActionsZalo(sessionId, currentRequest, message, nextContext, i - 1)
          );
        });
      } else {
        logger.debug('unknown response type ' + json.type);
   ...
   this.runActionsZalo = function(sessionId, message, context, maxSteps) {		
    ...
    return this.converse(sessionId, message, context, currentRequest > 1).then(		
      continueRunActionsZalo(sessionId, currentRequest, message, context, steps)		
    ).then(cleanup);
  };
